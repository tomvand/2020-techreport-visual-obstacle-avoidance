...
opt = format_tudelft_report;

% Load csv file
% [file, path] = uigetfile('*.csv');
file = 'evo_flight_11.csv';
path = pwd;
...
tab_evo(table);
set(gcf, opt.size.full);
xlim([50 110]);
ylim([-1 1]);
saveas(gcf, 'evo_velocity', opt.format);

figure('Name', 'Trajectory');
tab_optitrack(table);
set(gcf, opt.size.full);
saveas(gcf, 'evo_trajectory', opt.format);
...
%     legend('show', 'AutoUpdate', 'off');
    ...
    xlabel('time [s]');
    ylabel('vz [m/s]');
    legend('show', 'AutoUpdate', 'off', 'Location', 'southeast');
    plot_in_flight(table);
    shade_condition(table, ~table.percevite_ok, [1 0 0]*0.2 + 0.8);
